...
const updateEndTime = updateByTime => state => {
  const timeLimit = getEndOfGameTime();
  const endOfGameTime = Math.min(state.endOfGameTime + updateByTime, timeLimit);
...
class Game extends React.Component {
  ...
  endGameWhenTimeEnd(prevState) {
    if (!this.didGameEndedNow(prevState)) 
      return false;
    this.clearCountdownInterval();
  }
  ...
  collectItem(name, type) {
    const {collectedItems} = this.state;
    const itemState = collectedItems.find(findItemByName(name)) || emptyItemState(name);
    const updatedItemState = this.updateItem(itemState);
  ...
  updateItem(itemState) {
    const itemDetails = this
      .letterList
      .find(findItemByName(itemState.name));
    ...
    updatedItem.bonusPoints += this.addBonusPoints(updatedItem.quantity, updatedItem.bonusPoints, itemDetails);

    return updatedItem;
  }

  addBonusPoints(quantity, currentBonusPoints, itemDetails) {
    const {
      points: basePoint,
      bonus = {}
    } = itemDetails;
    ...
    return (
      <GameView>
        ...
        <PointsColumn>
          ...
          <Points collectedItems={collectedItems} startNewGame={this.startNewGame}/>
        </PointsColumn>
      </GameView>
    );
  }
